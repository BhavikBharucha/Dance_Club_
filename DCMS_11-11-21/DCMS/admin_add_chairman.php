...
<html>
    <head>
        <title>Add Chairman | Dance Club Management System</title>
        <?php
        include './head.php';
        ?>
        <style>
...
if (isset($_POST['btnadd'])) {
    $q_check = "SELECT COUNT(*) as total FROM tblmember WHERE role='Chairman' and status='A'";
    $r_check = mysqli_query($con, $q_check);
    $row_check = mysqli_fetch_assoc($r_check);
    if ($row_check['total'] >= 1) {
        echo "<script LANGUAGE='JavaScript'>
    window.alert('Maximum Number of Chairman Reached !');
    window.location.href='admin_manage_chairman.php';
    </script>";
        //header("Location:admin_manage_chairman.php");
    } else {
        $p=$_POST['txtpwd'];
        $pass_hash= password_hash($p, PASSWORD_DEFAULT);
        $q = "INSERT INTO tblmember (firstname, middlename, lastname, city, dob, gender, contactno, email, password, instituteid, role, status) VALUES ('" . $_POST['txtfname'] . "','" . $_POST['txtmname'] . "','" . $_POST['txtlname'] . "','" . $_POST['city'] . "','" . $_POST['dob'] . "','" . $_POST['gender'] . "','" . $_POST['txtnumber'] . "','" . $_POST['txtemail'] . "','$pass_hash','" . $_POST['institute'] . "','Chairman','A')";
