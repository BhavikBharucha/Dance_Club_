...
if ($_SESSION['adminpages'] != 1 && $_SESSION['chairmanpages'] != 1 && $_SESSION['coChairmanpages'] != 1) {
    ...
    header("Location: event_report.php");
    exit();
}
...
<html>
    <head>
        ...
        //include './head.php';
        ?>
        <title>Event Report | Dance Club Management System</title>
        <meta charset="utf-8">
                ...
                <section>
                    <center>
                        <div style="border: 3px solid darkgray;width: 886px; padding: 22px; border-radius: 7px; margin-top: 50px; margin-bottom: 50px;">
                            ...
                            <h4 style="text-align: center;">Event Report</h4>
                            ...
                            <table style="width: 830px;">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php
                                        $q = "select e.name,m.firstname from ((tbleventcoordinator ev INNER JOIN tblevent e on ev.eventid = e.id) INNER JOIN tblmember m on ev.memberid = m.id) where e.id='" . $_SESSION['eventname'] . "'";
                                ...
                                <tr>
                                    ...
                                    <td><?php
                                        $s = "select j.firstname,e.name from ((tbleventjudge ej INNER JOIN tbljudge j on ej.judgeid = j.id) INNER join tblevent e on ej.eventid = e.id) where e.id='" . $_SESSION['eventname'] . "'";
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php
                                        $sql = "select e.name,evc.weightage,c.criteria from ((tbleventcriteria evc INNER join tblevaluationcriteria c on evc.evaluationCriteriaId = c.id) INNER join tblevent e on evc.eventid = e.id) where e.id='" . $_SESSION['eventname'] . "'";
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <table>
                                            <tr>
                                                <?php
                                                $q = "SELECT AVG(es.team_score) AS a,es.teamid  FROM tbleventscore es INNER JOIN tbleventjudge ej ON es.eventJudgeid=ej.id WHERE ej.eventid='" . $_SESSION['eventname'] . "' GROUP BY es.teamid,es.eventJudgeid ORDER BY a asc";
                                                $r = mysqli_query($con, $q);
                                                $id_arr = array();
                                                $avg_arr = array();
                                                $sum_arr = array();
                                                $i=0;
                                                $arr_check=array(2,6,9,12,15);
                                                while ($row = mysqli_fetch_row($r)) {
                                                    array_push($id_arr, $row[1]);
                                                    if($row[1]==$id_arr[$i] ){
                                                        echo "thay che $i<br>";
